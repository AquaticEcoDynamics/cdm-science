...
```{r 3dev-sedvars200, echo=FALSE, message=FALSE, warning=FALSE}
 library(knitr)
 library(kableExtra)
 library(readxl)

 sediment_vars100 <- read_xlsx("tables/BookdownSDGVars.xlsx", sheet = "Vars" )
 kable(sediment_vars200,"html", escape = F, align = "c", caption = "All sediment parameters",
...
The linked variables are listed in the table. They can be linked by concentration, dissolved flux and particle flux. Salinity has only one possible salinity link, which is hard-coded in the sediment model. The SO~4~^2-^ link is hard-coded such that SO~4~^2-^ concentration in mmol L^-1^ is 0.9 × salinity (PSU). 
...
Most chemical species are initialized at zero concentration. Some species are initialised to meet field data (table below).
...
The flux of Fe(OH)~3A~ from the water column to the sediment is set from the parameter 'default_vals'. In the water column model, Fe is not simulated, therefore one value was applied equally to all zones. The value was calibrated to fit the Fe^2+^ concentration value.
...
kable(sediment_SecondaryRedox ,"html", escape = F, align = "c", caption = "Secondary redox parameters",      bootstrap_options = "hover") %>%
   kable_styling(sediment_initial, bootstrap_options = "hover",
               full_width = F, position = "center",
               font_size = 12) %>%
 column_spec(1, width_min = "10em") %>%
