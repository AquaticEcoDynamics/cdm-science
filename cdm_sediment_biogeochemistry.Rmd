...
All simulated variables are listed in the table below.
...
```{r 3dev-sedvars, echo=FALSE, message=FALSE, warning=FALSE}
 ...
 kable(sediment_vars,"html", escape = F, align = "c", caption = "All sediment variables",
   ...
   scroll_box(width = "200px", height = "20px",
              fixed_thead = FALSE)
```

##### Linked variables {.unnumbered}

The linked variables are listed in the table. They can be linked by concentration, dissolved flux and particle flux. Salinity has only one possible salinity link, which is hard-coded in the sediment model. The $SO_4^2-^$ link is hard-coded such that $SO_4^2-^$ concentration in mmol L^-1^ is 0.9 × salinity (PSU).
...
Most chemical species are initialized at zero concentration. Some species are initialised to meet field data (table below). The calibration is outlined in the Calibration section below.
...
The fundamental timing parameters are given in the table below. These set the start and end times, the spin up period and the rates of how often the model runs. These parameters are set in either the tuflow fvc file or the aed.nml.
...
The physical properties of the sediment are given in tables below. Some parameters are set as constant values for all zones, while others are set to be variable in each zone. The variability of bioturbation and irrigation along the length of the lagoon is set using the depth of the effect ($x_1$, $x_2$ and $x_{irrig}$) rather than the intensity ($DB_0$, $\alpha_0$). Upper porosity $p_0$ is held at 0.9 (i.e. 90% porewater) for all zones and the lower porosity $p_{00}$ is based on measured data at sites in each zone.
...
Secondary redox reactions are formed from the by-products of primary redox reactions. The nitrogen redox cycle parameters are given in the table below.
...
Secondary redox parameters for other processes are given in the table below.
...
Equilibrium and precipitation/dissolution constants are given in the table below.
...
kable(sediment_parameters,"html", escape = F, align = "c", caption = "Sediment physical and transport parameters",
  ...
  scroll_box(width = "690px", height = "200px",
             fixed_thead = FALSE)
```
...
```{r dev-picsbudgetsetup_1, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Major N pools as transferred between the water and sediment models."}

knitr::include_graphics("images/dev/sed_N_Overall.png")

```

#### Salinity and denitrification
...
```{r dev-picsbudgetsetup_2, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Salinity factor $F_{Sal}$ scaling between 1 and 0 between parameters *Sal_1* and *Sal_2*"}
...
+----------------------+-----------------------------------------------+
| IF Salinity \< Sal_1 | F_Sal = 1                                     |
|                      |                                               |
| \\nonumber           | \\nonumber                                    |
+======================+===============================================+
| IF Salinity \> Sal_1 | Sal_2 - Salinity / Sal_2 - Sal_1              |
|                      |                                               |
| \\nonumber           | \\nonumber                                    |
+----------------------+-----------------------------------------------+
| IF Salinity \> Sal_2 | F_Sal = 1                                     |
|                      |                                               |
| \\nonumber           | \\nonumber                                    |
+----------------------+-----------------------------------------------+
...
\end{eqnarray}
```
As with $F_{Sal}$, $F_{Sul}$ scales between 1 and 0, however, as a decay rather than a stepped function. The value for *K~H2S~* can be found in the physical parameters table above. In this simulation, the same value for all was used in all zones.

```{r dev-picsbudgetsetup_3, echo = FALSE, out.width='50%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Salinity factor $F_{Sal}$ scaling between 1 and 0 between parameters *Sal_1* and *Sal_2*"}
...
#### Sediment-water interface fluxes
...
Bioturbation was set up using the step-wise function, where *x~1~* was the depth of full bioturbation, and *x~2~* the depth of zero bioturbation. The values for *x~1~* and *x~2~* can be found in the physical parameters table above. In this simulation, the values were assigned for each zone, with deeper vales for the northern zones. The rate constant *DB~0~* was assigned to be the same for all zones. The original model had a constant bioturbation rate assigned at the initialisation step. For this simulation the model was developed to have a dynamic bioturbation rate that is updated at every time step.

```{r dev-picsbudgetsetup_4, echo = FALSE, out.width='50%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Salinity factor $F_{Sal}$ scaling between 1 and 0 between parameters *Sal_1* and *Sal_2*"}
...
$PO_{4 _{Particulate}}^{3-}}$ = $\frac {K_{PO_4p} ($Fe(OH)_{3A}$ + $Fe(OH)_{3B}$)} {1 + K_{PO_4p} ($Fe(OH)_{3A}$ + $Fe(OH)_{3B}$) } $PO_{4 _{Total}}^{3-}}$
\
$PO_{4 _{Dissolved}}^{3-}}$   = $\frac {1} {1 + K_{PO_4p} ($Fe(OH)_{3A}$ + $Fe(OH)_{3B}$) } $PO_{4 _{Total}}^{3-}}$

\end{eqnarray}
```
The final feature developed for nutrient burial for this project was the very unreactive organic matter parameter. As mentioned in the model setup above, the organic matter setup in this model has labile and refractory phases, as state variables. The model was extended in this project by dropping the reactivity of the particulate refractory phase towards zero as the concentration approaches a minimum. For $PON_R_$, the parameter *ponu* set the minimum concentration and *KOMPres_N* set the sensitivity of the decrease, as in the equation below. The equivalent parameters for C an P were *pocu* and *popu*, and *KOMPres_C* and *KOMPres_P*.
...
```{r dev-picsbudgetsetup_5, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "$O_2$ injected into the sediment from $MPB$ and plant roots."}
...
The total mass of TN and TP in both the north and the south lagoons was calibrated to the right order of magnitude as that measured in the field measurements. The total mass decreased by a small amount over the course of the simulation.
...
The fluxes were generally within the right order of magnitude. Comparison was made with either the constant values used by the water column model for calibration, and the measured fluxes. Constant values were available for $O_2$, $NO_3^-$, $PO_4^{3-}$ and $NH_4^+$. Zone 52 had the most measured flux data, from Parnka Point and measured under dark and light conditions, with measurements for $O_2$, $NO_3^-$, $PO_4^{3-}$, $NH_4^+$, $N_2O$ and $CH_4$. Simulated fluxes were generally closer to the measured values than the constant values for $O_2$, $PO_4^{3-}$, $NH_4^+$. In the case of $NO_3^-$, the constant flux was close to the measured fluxes.

```{r dev-calpics5, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Sediment-water fluxes in zone 52. Measured values are averages of several measurements for dark and light conditions."}
...
Simulated depth profiles of $NH_4^+$ matched the measured values well, in particular in zone 73. This was a major result, considering the very large total mass of N tied up in $PON_R$ and the relatively small amount produced as $DON$ and $NH_4^+$.
...
```{r dev-calpics6, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Ammonium concentration profile for zone 73, compared to data at Noonameena."}

knitr::include_graphics(c("images/dev/sed_C_D_core_zone73_amm_2020-11-30.png"))

```

Some measured $Fe^{2+}$ and $H_2S$ data showed a clear spike in the profile at around 10 cm (with the $H_2S$ spike being slightly deeper in the sediment than the $Fe^{2+}$ spike). The sediment model did not capture this, and so a future calibration would require getting the total magnitude of both species right, as well as the right depth of $O_2$ penetration. Since the $O_2$ was generally slightly too deep, it is assumed that a shallower $O_2$ depth would create the right anoxic zone for Fe and SO\~4^2-^ reduction.

```{r dev-calpics7, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Iron concentration profile "}

knitr::include_graphics(c("images/dev/sed_C_D_core_zone33_feii_2020-11-01.png"))

```

```{r dev-calpics8, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Sulfide concentration profile "}
...
\
Note that the bioturbation and bioirrigation input parameters were 40 and 200, respectively (see figure below). In the southern zones, these raw inputs were decreased to around 8 for bioturbation and 40 for bioirrigation. The image from Zone 52, which is the intermediate area between the north and south lagoons, illustrates how the dynamic salinity boundary produced a dynamic response from biota-driven mixing. Future work could build in a time lag between the onset of the environmental condition and the result for the benthic fauna.

```{r dev-nutpics3, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Salinity and sulfide inhibition functions in four of the sediment zones."}

knitr::include_graphics(c("images/dev/sed_FSalFSulBioturb-01.png"))

```

As an example going across the breadth of the lagoon, bioturbation and bioirrigation in zones 61 to 63 are shown in the figure below. The depth and magnitude of bioturbation are smaller in the central, deep zone 62, than on the shallow edge zones, 61 and 63.

```{r dev-nutpics2, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Salinity and sulfide inhibition functions in four of the sediment zones."}

knitr::include_graphics(c("images/dev/sed_Z6Bioturb.png"))

```

The total mass of TN flux across the sediment-water interface was calculated for one year. The flux was dominated by the influx of $PON$ from the water to the sediment. There was a relatively smaller amount of $NH_4^+$ outflux from the sediment to the water.\
*Excel figure of summed fluxes, TN North, South*
...
Although the influx of $PON$ was a far larger amount than the outflux of $NH_4^+$, there was an overall loss of $PON_R$ in the upper 5 cm (see figure). The $PON_R$ was hydrolysed to $DON_R$, which was mineralised to $NH_4^+$. Deeper than 10 cm, the $PON_R$ concentration was stable over time. The $DON_R$ concentration was dependent on the flux to the water column.

```{r dev-nutpics5, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Upper 25 cm of sediment over 50 days for zone 52."}

knitr::include_graphics(c("images/dev/sed_t_D_C_3N_zone_52.png"))

```

```{r dev-nutpics6, echo = FALSE, out.width='75%', class = "text-image",fig.show='hold',fig.align='center', fig.cap = "Sediment-water interface fluxes for major N species."}
...
The change in mass at the 5 cm depth over a year was very small (around 1%) for both lagoons and for both $TN$ and $TP$. Using this definition of burial, very little of the nutrients were removed.\
The concentration of adsorbed $NH_4^+$ and $PO4^{3-}$ was also a small percentage of TN and TP, and stayed constant over the course of the simulation (see, for example the figure below). It was not the case that adsorption was a major process for removing water column nutrients and locking them in the sediment. In the case of adsorbed $PO4^{3-}$, the lack of change in concentration was due, in part, to the relatively low amount of Fe reduction in the sediment.
